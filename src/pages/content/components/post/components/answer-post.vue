<template>
  <view v-if="item.contentType === 4">
    ...
    <view class="mb_15" v-if="item.medias">
      <sound-playing
        v-if="item.contentMedia === 4"
        :audio="item.medias[0]"
        :statistic-data="item"
        :content-id="item.contentId"
      ></sound-playing>
      <video-playing v-else-if="item.contentMedia === 3" :video="item.medias[0]" :statistic-data="item"></video-playing>
    ...
    <view>
      <view class="flex flex-between flex-middle">
        <view class="flex flex-middle flex-left">
          <view class="collect">
            <get-bt border :data="item" @click="$store.commit(SET_GET_TITLE, item.content)"></get-bt>
          </view>
    ...
    <!--  问题  -->
    <question v-if="postType !== 'recommend'" :item="item"></question>
    <!-- 评论或回答  -->
...
export default {
  ...
  methods: {
    ...
    init(item) {
      const { comments, isFollow, circleId, circleName, circleUserNumber, circleFollow, circleHotNumber, circleImg } =
        item;
